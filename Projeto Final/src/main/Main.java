...
public class Main {
            ...
            switch (opcaoMenuEscolhida) {
                ...
                case 1:
                    if (listaDeAtletas.size() < 11 || listaDeAtletas.size() > 11) {
                        ...
                        break;
                    } else if (verificaSeTemCamisaRepetida(listaDeAtletas) == true) {
                        JOptionPane.showMessageDialog(null, "Os jogadores não podem ter camisas iguais");
                        break;
                    } else {
                        ...
                        do {
                            ...
                            switch (opcaoMenuEscolhida) {
                                ...
                                case 1:
                                    ...
                                    gerenciadorDePartida.setNomeAdversario(JOptionPane.showInputDialog("Nome do adversário: "));

                                    gerenciadorDePartida.setDataPartida(LocalDate.of(Integer.parseInt(dataDaPartida[2]),
                                            Integer.parseInt(dataDaPartida[1]),
                                            Integer.parseInt(dataDaPartida[0])));

                                    gerenciadorDePartida.iniciarPartida(gerenciadorDePartida.getDataPartida(), gerenciadorDePartida.getNomeAdversario(), listaDeAtletas);
                                    break;

                                case 2:
                                    if (gerenciadorDePartida.getNomeAdversario() == null) {
                                        ...
                                        int jogadorEscolhido = Integer.parseInt((String) JOptionPane.showInputDialog(null, "Escolha um jogador:\n" + listarJogadoresPorNome(listaDeAtletas), "CLUBE DE REGATAS FLAMENGO",
                                                JOptionPane.INFORMATION_MESSAGE, Menus.logoFlamengo(), null, ""));
    ...
    public static boolean verificaSeTemCamisaRepetida(ArrayList<Atleta> listaDeAtletas) {
        int[] camisas = new int[listaDeAtletas.size()];

        for (int i = 0; i < camisas.length; i++) {
            camisas[i] = listaDeAtletas.get(i).getNumeroCamisa();
        }

        for (Atleta atleta : listaDeAtletas) {

            int contaQuantidadeCamisa = 0;

            for (int i = 0; i < camisas.length; i++) {

                if (atleta.getNumeroCamisa() == camisas[i]) {
                    contaQuantidadeCamisa += 1;
                }

            }

            if (contaQuantidadeCamisa > 1) {
                return true;
            }
